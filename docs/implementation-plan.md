...
- CLI 구현 언어와 배포 방식을 선택한다.
- Server·Agent의 health, 상태 조회, 기록 수집 가능 여부를 조사한다.
- `BENCHMARK_MODE`의 책임과 데이터 초기화 범위를 정한다.
- 첫 번째 테스트 저장소와 단일 리포트를 선정한다.
...
- 구현 언어와 패키징 방식
- MVP에서 사용할 테스트 저장소와 리포트
- Server·Agent의 기존 API 중 재사용 가능한 범위
- `BENCHMARK_MODE`에서 추가할 endpoint와 초기화 방식
- 로컬에서 필요한 비밀 값과 전달 방식
